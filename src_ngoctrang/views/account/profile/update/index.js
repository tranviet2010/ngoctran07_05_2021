...
class UpdateInformation extends Component {
  ...
  updateAccount = () => {
    ...
    if (userName.trim() === "" || userName.length > 50 || !checkFullName(userName)) {
      ...
      return Alert.alert(
        "Thông báo",
        "Địa chỉ không nhập quá 100 ký tự, không bao gồm các ký tự đặc biệt",
        // () => this.focusPhone.focus()
      );
    }
    else if (
      ...
      Alert.alert(
        "Thông báo",
        "Số tài khoản chỉ gồm số và nhỏ hơn 20 chữ số",
      );
    }
    else if (
      ...
      Alert.alert(
        "Thông báo",
        "Tên tài khoản chỉ gồm chữ, không chứa ký tự đặc biệt và nhỏ hơn 50 kí tự",
