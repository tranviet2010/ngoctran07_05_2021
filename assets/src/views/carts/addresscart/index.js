...
class DetailAddressCart extends Component {
  ...
  changeCity = (text) => {
    console.log("check city", text);
    if (text == "- None -") {
      this.setState({ city: "", district: "", districChild: "" });
    } else {
      this.setState({ city: text, district: "", districChild: "", checktinh: false ,checkquan:true, checkxa:true}, () => {
      });
    }
  };
  changeDistrict = (text) => {
    ...
    } else this.setState({ district: text, districChild: "",checkquan:false, checkxa:true });
  };
  changeDistrictChild = (text) => {
    if (text == "- None -") {
      this.setState({ districChild: "" });
    } else this.setState({ districChild: text,checkxa:false });
  };
  checkTime = (a, b) => {
    var start = a;
    var end = b;
      ...
      this.setState(
        ...
        async () => {
          ...
          orderProduct({
            ...
            ID_CITY: typeOrder?'':city.MATP,
            PAYMENT_TYPE: Numbercode,
            ID_DISTRICT: typeOrder?'':district.MAQH,
            ADDRESS: typeOrder?'':address,
            ID_WARD: typeOrder?'':districChild.XAID,
            IDSHOP: 'ABC123',
          })
            .then((result) => {
      ...
      <ScrollView>
        ...
        >
          <View style={styles.infor}>
            ...
            {this.props.authUser.GROUPS == 8 ? null : <Text
             onPress={() =>
               this.setState({ check: false, checktinh: false,checkquan:false,checkxa:false })
              }
              ...
              {value ? null : <View style={{ alignSelf: "center", marginTop: sizeHeight(1) }}>
                ...
                <FormTextInput
                  props={{
                    ...
                    // onChangeText: (text) => {mDistrict=text},
                    primary: "#017DFF",
                    color: COLOR.BUTTON,
                    // onDelete: () => null,
                    style: styles.styleWidth,
                ...
                <FormTextInput
                  props={{
                    ...
                    // onChangeText: (text) => {mAddress=text},
                    primary: "#017DFF",
                    color: COLOR.BUTTON,
